...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    /* ================================================================= */
    /* ----------------------------------------------------------------- */
    int udscid2 = dm_api_udsc_create(om_api, &config);
    if (udscid2 >= 0) {
       demo_printf("UDS client create success => %d \n", udscid2);
    }
    ...
    service.timeout = 100;
    /* 添加一个诊断请求 */
    if (dm_api_udsc_service_config(om_api, udscid2, &service) == 0) {
        demo_printf("UDS service config success. \n");
    }
    /* UDS服务项添加结束，释放内存 */
    ByteArrayDelete(service.variableByte);
    ...
    dm_api_udsc_general_config(om_api, udscid2, &gconfig);
    
    /* M这个函数是必须调用的 接收OTA MASTER的诊断请求报文并通过doIP或者doCAN发送 */
    dm_api_udsc_request_transfer_callback_set(om_api, udscid2, dm_uds_request_transfer_callback, 0/* 用户数据指针 */);
    
    /* M这个函数是必须调用的 启动UDS客户端开始执行诊断任务 */
    dm_api_udsc_start(om_api, udscid2);
